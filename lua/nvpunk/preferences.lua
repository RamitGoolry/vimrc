...
local function select_format_get_label(item)
	return item.label
end
...
local preferences_menus = {
	...
	},
	{
		...
		func = function()
			vim.ui.select(require('nvpunk.plugins_conf.headers').headers, {
				...
				},
				{
					...
					func = function()
						vim.ui.select({
							...
							M.set_statusline_style(item.value)
						end)
					end,
				},
				{
					...
					func = function()
						vim.ui.select(BORDER_SELECT_OPTS, {
							prompt = 'Window Borders:',
							format_item = select_format_get_label,
						}, function(item, _)
							M.set_window_border(item.value)
						end)
					end,
				},
				{
...
M.preferences_menu = function()
	vim.ui.select(preferences_menus, {
		prompt = 'Preferences:',
		format_item = select_format_get_label,
	}, function(item, _)
		item.func()
...
vim.api.nvim_create_user_command('NvpunkPreferences', function(_)
	M.preferences_menu()
end, { nargs = 0 })

vim.api.nvim_create_user_command('Theme', function(_)
	require('nvpunk.theme_manager.telescope_theme_chooser')()
end, { nargs = 0 })
